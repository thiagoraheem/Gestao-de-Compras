...
const ReceiptPhase = forwardRef((props: ReceiptPhaseProps, ref: React.Ref<ReceiptPhaseHandle>) => {
  ...
  const confirmFiscalMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Sucesso", description: "Conferência fiscal registrada!" });
      if (request.phase === 'conclusao_compra') {
      }
    },
    onError: (err: any) => {
      toast({ title: "Erro", description: err.message || "Erro ao confirmar fiscalmente", variant: "destructive" });
    }
  });
      ...
      <div className={cn("space-y-6", className)}>
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {canPerformReceiptActions && mode === 'fiscal' && (
              <Button
                onClick={() => confirmFiscalMutation.mutate()}
                disabled={confirmFiscalMutation.isPending}
              >
                <Check className="w-4 h-4 mr-2" />
                Concluir Conf. Fiscal
              </Button>
            )}
            {canPerformReceiptActions && mode !== 'fiscal' && (
              <Button 
                onClick={() => setViewMode('new_receipt')} 
                disabled={purchaseOrder?.fulfillmentStatus === 'fulfilled'}
              >
        ...
        <PurchaseRequestHeaderCard
          context={mode === 'fiscal' ? "fiscal" : "physical"}
