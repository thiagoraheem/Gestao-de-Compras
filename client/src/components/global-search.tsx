...
export default function GlobalSearch({ className }: GlobalSearchProps) {
  ...
  const suggestions = useMemo(() => {
    ...
    const isRequestNumberSearch = query.match(/^(r|sol|req)?[-\s]?(\d+)/i) || query.match(/^\d+$/);
    if (isRequestNumberSearch) {
      // Extrair apenas os números da pesquisa para busca flexível
      const numbers = query.replace(/[^\d]/g, '');
      const matchingRequests = purchaseRequests.filter(req => 
    ...
    const textMatchingRequests = purchaseRequests.filter(req => 
  ...
  const getSearchTypeHint = (query: string) => {
    ...
    if (query.includes("@") || query.match(/\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}/)) {
            ...
            <CommandList className="max-h-80">
              {suggestions.length === 0 ? (
                <CommandEmpty className="py-6 text-center text-sm">
                  {searchValue ? "Nenhum resultado encontrado." : "Digite para pesquisar..."}
                </CommandEmpty>
              ) : (
                <CommandGroup>
