...
export function AdminSuperUser() {
  ...
  const searchMutation = useMutation({
    mutationFn: async (requestNumber: string) => {
      return await apiRequest(`/api/admin/purchase-requests/search/${encodeURIComponent(requestNumber)}`);
    },
    onSuccess: (data) => {
      ...
      toast({ title: "Sucesso", description: "Solicitação encontrada e carregada para edição" });
    },
    onError: (error: any) => {
      toast({ title: "Erro", description: error?.message || "Solicitação não encontrada", variant: "destructive" });
    },
  });

  const updateMutation = useMutation({
    mutationFn: async (data: any) => {
      return await apiRequest(`/api/admin/purchase-requests/${currentRequest.id}`, {
        method: "PATCH",
        body: data,
      });
    },
    onSuccess: () => {
      setCurrentRequest((prev: any) => ({ ...prev, ...formData }));
      toast({ title: "Sucesso", description: "Solicitação atualizada com sucesso" });
      queryClient.invalidateQueries({ queryKey: ["/api/purchase-requests"] });
    },
    onError: (error: any) => {
      toast({ title: "Erro", description: error?.message || "Falha ao atualizar solicitação", variant: "destructive" });
    },
  });

  const handleSearch = () => {
    if (!searchNumber.trim()) {
      toast({ title: "Erro", description: "Informe o número da solicitação", variant: "destructive" });
      return;
    }
  ...
  const addItem = () => {
    setFormData({
      ...formData,
      items: [...(formData.items || []), { description: "", unit: "", requestedQuantity: "1", technicalSpecification: "" }]
    });
  };
              ...
              <Tabs defaultValue="edit" className="w-full">
                <TabsList className="grid w-full grid-cols-2">
                  <TabsTrigger value="edit" className="flex items-center gap-2">
                    <Save className="h-4 w-4" /> Editar Solicitação
                  </TabsTrigger>
                  <TabsTrigger value="audit" className="flex items-center gap-2">
                    <Shield className="h-4 w-4" /> Auditoria de Dados
                  </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="edit" className="space-y-6 mt-6">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        disabled data-testid="requester-name"
                      />
                    </div>
                    <div>
                      ...
                      <Input
                        ...
                        disabled data-testid="cost-center"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div>
                        ...
                        <Select value={formData.urgency} onValueChange={(value) => setFormData({...formData, urgency: value})}>
                          <SelectTrigger data-testid="select-urgency"><SelectValue placeholder="Selecione..." /></SelectTrigger>
                          <SelectContent>
                      ...
                      <div>
                        ...
                        <Select value={formData.currentPhase} onValueChange={(value) => setFormData({...formData, currentPhase: value})}>
                          <SelectTrigger data-testid="select-phase"><SelectValue placeholder="Selecione..." /></SelectTrigger>
                          <SelectContent>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <Input id="idealDeliveryDate" type="date" data-testid="input-delivery-date"
                      ...
                      <div>
                        ...
                        <Input id="availableBudget" type="number" step="0.01" data-testid="input-budget" placeholder="0.00"
                    ...
                    <div>
                      ...
                      <Textarea id="justification" data-testid="textarea-justification"
                        value={formData.justification}
                        onChange={(e) => setFormData({...formData, justification: e.target.value})} rows={3} />
                    </div>
                    <div>
                      <Label htmlFor="additionalInfo">Informações Adicionais</Label>
                      <Textarea id="additionalInfo" data-testid="textarea-additional-info"
                        value={formData.additionalInfo}
                        onChange={(e) => setFormData({...formData, additionalInfo: e.target.value})} rows={3} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <Button onClick={addItem} variant="outline" size="sm" data-testid="button-add-item">Adicionar Item</Button>
                    </div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center justify-between">
                            ...
                            <Button onClick={() => removeItem(index)} variant="destructive" size="sm"
                              data-testid={`button-remove-item-${index}`}>Remover</Button>
                          </div>
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <div>
                              <Label>Descrição</Label>
                              <Input value={item.description || ""} data-testid={`input-item-description-${index}`}
                                onChange={(e) => updateItem(index, "description", e.target.value)} />
                            </div>
                            <div>
                              <Label>Unidade</Label>
                              <Input value={item.unit || ""} data-testid={`input-item-unit-${index}`}
                                onChange={(e) => updateItem(index, "unit", e.target.value)} />
                            </div>
                            <div>
                              <Label>Quantidade</Label>
                              <Input type="number" step="0.01" value={item.requestedQuantity || ""}
                          ...
                          <div>
                            ...
                            <Textarea value={item.technicalSpecification || ""} rows={2}
                ...
                <TabsContent value="audit" className="mt-6">
                  <DataAudit requestId={currentRequest.id} requestNumber={currentRequest.requestNumber} />
                </TabsContent>
              </Tabs>
            </CardContent>
          </Card>
