...
const STATUS_STYLES: Record<string, string> = {
  rascunho: "bg-gray-500", nf_pendente: "bg-yellow-500", nf_confirmada: "bg-blue-500",
  validado_compras: "bg-indigo-500", enviado_locador: "bg-purple-500", integrado_locador: "bg-green-500",
  erro_integracao: "bg-red-500", recebimento_confirmado: "bg-green-600",
};
const STATUS_LABELS: Record<string, string> = {
  rascunho: "Rascunho", nf_pendente: "Pendente", nf_confirmada: "Confirmada",
  validado_compras: "Validado", enviado_locador: "Enviado ERP", integrado_locador: "Integrado",
  erro_integracao: "Erro Integração", recebimento_confirmado: "Recebido",
};

const StatusBadge = ({ status }: { status: string }) => (
  <Badge className={cn(STATUS_STYLES[status] || "bg-gray-500")}>{STATUS_LABELS[status] || status}</Badge>
);
...
const ReceiptItemsList = ({ receiptId }: { receiptId: number }) => {
  ...
  return (
    <div className="bg-muted/30 p-4 rounded-md mx-4 mb-4 border border-muted">
      <h4 className="text-sm font-semibold mb-2 flex items-center gap-2"><FileText className="h-4 w-4" /> Itens da Nota</h4>
      <Table>
        <TableHeader>
        ...
        <TableBody>
          {items.map((item) => (
            <TableRow key={item.id} className="hover:bg-muted/50">
              ...
              <TableCell className="text-right text-xs">{formatCurrencyLocal(item.unitPrice)}</TableCell>
              <TableCell className="text-right text-xs font-semibold">{formatCurrencyLocal(item.totalPrice)}</TableCell>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Card>
                ...
                <CardContent className="text-sm space-y-1">
                  ...
                  <p><span className="font-semibold">Valor Total:</span> {formatCurrencyLocal(receipt?.totalAmount || 0)}</p>
                </CardContent>
              </Card>
              <Card>
...
export function InvoicesReport() {
  ...
  const [filters, setFilters] = useState({ number: "", series: "", supplierName: "", startDate: "", endDate: "" });
  const [expandedRows, setExpandedRows] = useState<number[]>([]);
  const [selectedReceipt, setSelectedReceipt] = useState<Receipt | null>(null);
  const [isVisualizerOpen, setIsVisualizerOpen] = useState(false);
  const [isFiltersCollapsed, setIsFiltersCollapsed] = useState(false);

  const { data, isLoading } = useQuery<InvoicesSearchResponse>({
    queryKey: ["invoices-search", page, filters],
    queryFn: async () => {
      const params = new URLSearchParams({ page: page.toString(), limit: "10", ...filters });
  ...
  const handleExport = () => {
    ...
    const csvContent = [
      headers.join(","),
      ...data.data.map((r: Receipt) => [r.documentNumber, r.documentSeries, `"${r.supplierName}"`, r.supplierCnpj, formatDate(r.documentIssueDate), formatDate(r.documentEntryDate), r.totalAmount, r.status].join(","))
    ].join("\n");
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="space-y-2">
                ...
                <Input placeholder="Ex: 12345" value={filters.number} onChange={(e) => handleFilterChange("number", e.target.value)} />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Fornecedor</label>
                <Input placeholder="Nome ou parte do nome" value={filters.supplierName} onChange={(e) => handleFilterChange("supplierName", e.target.value)} />
              </div>
              <div className="space-y-2">
            ...
            <div className="mt-4 flex justify-end gap-2">
              <Button variant="ghost" onClick={() => { setFilters({ number: "", series: "", supplierName: "", startDate: "", endDate: "" }); setPage(1); }}>
                Limpar Filtros
              </Button>
            </div>
          </CardContent>
      ...
      <Card>
        <CardContent className="p-0">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow><TableCell colSpan={8} className="h-24 text-center"><div className="flex justify-center items-center gap-2"><Loader2 className="animate-spin h-5 w-5" /> Carregando...</div></TableCell></TableRow>
              ) : data?.data?.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="h-24 text-center text-muted-foreground">Nenhuma nota fiscal encontrada.</TableCell></TableRow>
              ) : (
                data?.data?.map((receipt: Receipt) => (
                  <>
                    <TableRow key={receipt.id} className={cn("group", expandedRows.includes(receipt.id) ? "bg-muted/50" : "")}>
                      <TableCell>
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => toggleRowExpansion(receipt.id)}>
                          {expandedRows.includes(receipt.id) ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                        </Button>
                      </TableCell>
                      ...
                      <TableCell className="text-right font-medium">{formatCurrencyLocal(receipt.totalAmount)}</TableCell>
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" onClick={() => { setSelectedReceipt(receipt); setIsVisualizerOpen(true); }}>
                    ...
                    {expandedRows.includes(receipt.id) && (
                      <TableRow className="bg-muted/10 hover:bg-muted/10">
                        <TableCell colSpan={8} className="p-0">
                          <div className="py-2"><ReceiptItemsList receiptId={receipt.id} /></div>
                        </TableCell>
                      </TableRow>
                    )}
                  </>
        ...
        {data?.pagination && (
          <div className="flex items-center justify-between px-4 py-4 border-t">
            ...
            <div className="flex gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => setPage(p => Math.min(data.pagination.totalPages, p + 1))} disabled={page >= data.pagination.totalPages}>Próxima</Button>
            </div>
          </div>
        )}
      </Card>
