...
export function registerQuotationRoutes(app: Express) {
  ...
  app.post("/api/quotations", isAuthenticated, async (req, res) => {
    try {
      ...
      const quotationNumber = `RFQ-${new Date().getFullYear()}-${String(quotationDataForApi.purchaseRequestId).padStart(3, "0")}`;
  ...
  app.delete("/api/quotations/:id", isAuthenticated, isAdminOrBuyer, async (req, res) => {
    try {
      ...
      if (!quotation) {
        return res.status(404).json({ message: "RFQ não encontrada" });
      }

      if (quotation.isActive) {
        return res.status(400).json({ message: "Não é possível excluir uma RFQ ativa" });
      }
      ...
      res.status(500).json({ message: "Erro ao excluir RFQ" });
    }
  });
  ...
  app.post("/api/quotations/:id/items", isAuthenticated, async (req, res) => {
    try {
      ...
      res.status(400).json({ message: "Failed to create quotation item" });
    }
  });
  ...
  app.post("/api/quotations/:id/send-rfq", isAuthenticated, async (req, res) => {
    try {
      ...
      if (!effectiveSupplierIds || effectiveSupplierIds.length === 0) {
        return res
          .status(400)
          .json({ message: "Pelo menos um fornecedor deve ser selecionado" });
      }
      ...
      try {
        ...
        if (selectedSupplierQuotation) {
          ...
          const recalculatedTotal = updatedSupplierItems.reduce((total, item) => {
            ...
            const discountedCandidate = item.discountedTotalPrice ? parseFloat(item.discountedTotalPrice) : NaN;
            const baseTotal = parseFloat(item.totalPrice || "0") || 0;
            const itemValue = hasItemDiscount && Number.isFinite(discountedCandidate) && discountedCandidate > 0 ? discountedCandidate : baseTotal;
          ...
          if (selectedSupplierQuotation.discountType === 'percentage' && selectedSupplierQuotation.discountValue) {
            calculatedTotal -= (calculatedTotal * parseFloat(selectedSupplierQuotation.discountValue)) / 100;
  ...
  app.put("/api/supplier-quotations/:id/mark-no-response", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      console.error("Error marking no response:", error);
      res.status(500).json({ message: "Failed to mark no response" });
    }
  });
  ...
  app.put("/api/supplier-quotations/:id/update-quantities", isAuthenticated, QuantityValidationMiddleware.fullValidation, async (req: Request, res: Response) => {
    try {
      ...
      if (!supplierQuotation) return res.status(404).json({ message: "Supplier quotation not found" });
      ...
      if (!atomicResult.success) {
        return res.status(400).json({ message: atomicResult.error || "Failed to update quantities", errors: atomicResult.errors || [] });
      }

      res.json({ message: "Quantities updated successfully", success: true });
    } catch (error: any) {
      console.error("Error in atomic quantity update:", error);
      res.status(500).json({ message: "Failed to update quantities", error: error?.message || String(error) });
    }
  });
  ...
  app.post("/api/quotations/:quotationId/versions/:targetVersion/rollback", isAuthenticated, async (req: Request, res: Response) => {
    try {
      ...
      const currentUser = await storage.getUser(req.session.userId!);
      if (!currentUser?.isAdmin && !currentUser?.isManager) return res.status(403).json({ message: "Insufficient permissions" });
